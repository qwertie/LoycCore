...
> The modernization work (swapping in BitOperations, Array.Copy, span comparisons, etc.) was supposed to be behavior-preserving. To check that, the old assemblies were kept as built binaries, and both old and new versions of a function were run side by side on large volumes of randomized inputs — on the order of millions of assertions — with any divergence flagged. Divergences then sorted into two piles: "the new code is wrong, fix the modernization" or "the old code was wrong all along." The second pile is where a lot of the ~50 came from. For genuinely suspect functions, the comparison was against an independent oracle instead of the old code — e.g. Math128.Multiply was checked against BigInteger, which is how "7.2% of uniform-random 64-bit multiplies are wrong" became a measured number rather than a guess.

**Loyc.Essentials**:

- **Possibly breaking**: `MessageSink.Default` and `MessageSink.ContextToString` overrides now use the new `AmbientService<T>` to flow across `await` points and thread-pool continuations; `SetDefault`'s return type changed from `SavedValue<IMessageSink>` to `AmbientService<IMessageSink>.Saved`.
- **Possibly breaking** bug fix: `StringBuilderExt.FirstIndexOf(char)` returns `null` instead of -1 when the character is not found, as documented and as the other overloads already did.
- Added `Loyc.WTF8Encoding`, a fast `System.Text.Encoding` subclass for [WTF-8](https://simonsapin.github.io/wtf-8/): UTF-8 extended so that unpaired UTF-16 surrogates encode as themselves (3 bytes),  to make *every* `string` round-trip losslessly through `byte[]` (`Encoding.UTF8` silently substitutes U+FFFD). Spans without surrogates are delegated to `Encoding.UTF8`'s vectorized code paths.
- Added `Memory2<T>`/`ReadOnlyMemory2<T>` (in `Loyc.SyncLib.Impl`): variants of `Memory<T>` whose `.Span` property is cheaper in arrays. Used to speed up the SyncLib readers.
...
- Bug fix: `ListExt.Randomize` now uses an unbiased Fisher–Yates shuffle.
- Bug fix: `ReferenceComparer<T>.GetHashCode` now uses `RuntimeHelpers.GetHashCode`, so a type that overrides `GetHashCode` can no longer break the reference-identity contract (mutating an object used as a key in a `Dictionary<T,V>` with this comparer no longer loses the entry).
- Added overload `AmbientService<T>.Set(newValue, alsoSetGlobalDefault)` that changes the fallback value for all threads while also setting the current async context's override. Also, `AmbientService<T>` no longer requires `T` to be a reference type.
- `UString`: Added `UString.AsSpan()`; ordinal `Equals`/`StartsWith`/`EndsWith`/`IndexOf` now use it with vectorized span comparisons; `ToUpper` has a fast path for no changes; `operator+` uses `string.Concat(span, span)` on .NET 5+.
- Bug fix: `InternalList<T>.InsertRange`/`DList<T>.InsertRange(int, IEnumerable<T>)` inserted the collection **twice** when the source implemented both `IReadOnlyCollection<T>` and `ICollection<T>`.
- Bug fix: `UString.Replace("", x)` looped forever; it now returns `this` as documented.
- Bug fixes: `ListExt.ReverseInPlace` always threw; `Repeated<T>.PopLast` recursed to `StackOverflowException` and its `Contains` ignored the count; `ListSourceAsSparse.GetEnumerator` also self-recursed.
- Bug fix: `SymbolPool` enumerators no longer hold the pool's lock across `yield return` — an abandoned enumerator could permanently deadlock every `GSymbol.Get` call. Tradeoff: snapshot is allocated per enumeration.
- Bug fix: `BaseDictionary.IsEmpty` was inverted, and its debugger display works again.
- Bug fixes in `G`: `Log2Floor(0)` returns -1 on all .NET targets, and `MakeValidFileName` actually replaces invalid characters.
- Bug fixes in EnumerableExt: `ConcatNow` returned the first element repeatedly; `AdjacentPairsCircular(IEnumerable)` mistakenly delegated to `AdjacentPairs`; `IndexOfMin`/`IndexOfMax` (which ignore nulls) incorrectly returned 0 if the min/max element was the first non-null element in the list.
- More bug fixes: `PrintHelpers` without `EscapeC.Control` failed to pass through control characters, writing them as decimal numbers instead; `MemoizedTypeName` threw for a non-generic type nested inside a generic type, e.g. `Outer<int>.Inner` (such types report `IsGenericType` but have no `` `1 `` suffix in their `Name`); in `MultiMap<K,V>`, `map[key].Clear()` forgot to subtract the removed values from `Count`, which over-reported from then on; `HashTags<V>.CopyTo` wrote its internally-cached pair over `array[arrayIndex]` after already copying it as part of the dictionary, losing one entry and duplicating another; `DList<T>`'s range `CopyTo` validated `subcount` against `Math.Max` of the remaining source/destination space instead of `Math.Min`, accepting an oversized count whenever *either* side had room (failing mid-copy instead of up front); `ByteArrayInString.Convert` ignored its `forceInitialEscape` argument; `GraphMethods.ScanComponent` recursed on `edge.To` (the current node) instead of `edge.From` when following inbound edges, so it never walked more than one hop upstream and silently returned only part of a weakly-connected component.
- Performance: hot loops in `InternalList`/`InternalDList` now use `Array.Copy`/`Array.IndexOf`/`Span.Clear`; `Maybe<T>` and `Either<L,R>` are now `readonly struct`s; `SequenceHashCode` and friends avoid redundant work.

**Loyc.Collections:**

- Performance: `AList`/`BList` node shifts use `Array.Copy` instead of element-by-element loops (measured 2.8–6.4× faster at realistic node sizes; with default node cap of 64).
- Performance: `VList` also uses `Array.Copy` now and `VList`/`FVList.Equals` no longer use exceptions as control flow (~20,000× faster when comparing mismatched types).
...
- **Conceivably breaking**: LES printers now use digit separators by default: decimal literals group digits by 3 (`1_234_567`), hex by 4, binary by 8. Technically this is a bug fix; the `digitSeparatorChar` constructor parameter of `StandardLiteralHandlers` was previously ignored. Pass `digitSeparatorChar: null` for the old behavior.
- **Conceivably breaking**: Bug fix: `LNode.Equals` string literals with different type markers (e.g. `re"x"` vs `str"x"`) wrongly compared equal in the default comparison mode.
...
- Bug fix: `Les2Lexer.ParseIdentifier` corrupted normal identifiers (`foo` became `102oo`), which wasn't noticed because it's only called to handle special identifiers. Also, `Les2Printer` broke astral characters such as emoji by truncating code points to `char`.
- Bug fix: `StreamCharSource` treated a short `Stream.Read` result as end-of-file, which would wrongly truncate input from network and decorator streams. Also, errors now throw `IOException` instead of base `Exception`.
...
- This package now also ships net6.0 and netstandard2.1 builds (it was netstandard2.0-only).
- Bug fix: `CPTrie`'s three-byte UTF-8 decoder corrupted every stored key containing a character ≥ U+0800; `CPBitArrayLeaf`'s clone constructor threw `NullReferenceException` for any dense node, and its `LastKeyInUse` scanned in the wrong direction. Also, `CPTrie.Contains` no longer throws for types that aren't `IComparable`.
- Bug fix: `UG.ProcessCommandLineArguments` applied a culture-sensitive `ToLower` that defeated `caseSensitiveLongOpts`. Separately, its `@optionsFile` expansion read the file via an undisposed `StreamReader`, keeping the file locked until the finalizer happened to run; it now uses `File.ReadAllText`.
...
- Bug fix: `SyncBinary` now stores strings in true [WTF-8](https://simonsapin.github.io/wtf-8/) (see `WTF8Encoding` above), so strings with unpaired UTF-16 surrogates round-trip losslessly; previously each unpaired surrogate was silently replaced with U+FFFD (of course, wire format is unchanged).
- Replaced `System.Runtime.Serialization.ObjectIDGenerator` with new public class `Loyc.SyncLib.Impl.ObjectIdGenerator` because the former is annoyingly marked Obsolete in .NET 8.
...
No one ever consumed this library before today, but here's the list of edits anyway:

- Added `SyncJsonDOM`, which reads JSON from a `System.Text.Json.JsonDocument` or `JsonElement` that you've already parsed (e.g. one that a web framework handed to you). Its `Read`/`ReadI`/`NewReader` methods mirror `SyncJson`, and `SyncJsonDOM.Reader` understands the same conventions as `SyncJson.Reader` (`NameConverter`, `$id`/`$ref` deduplication and object cycles, Base64/BAIS byte arrays, type tags), so it can read anything `SyncJson.Writer` wrote. Because the DOM is random-access, reading fields out of order is cheap, and a `$ref` can even refer to an `$id` that appears later in the document (the streaming reader can't do that). `SyncJsonDOM` exists only in the net6.0 build, since `JsonDocument` is unavailable in .NET Standard 2.x / .NET Framework.
- Optimized `SyncJson.Reader`: faster integer parsing; faster string encoding/decoding (vectorized via `Encoding.ASCII` and SIMD); floating-point numbers now parse/print via `Utf8Parser` and `Utf8Formatter` on .NET 6+: integer writes are 2.69× faster with no per-value string allocations, and parsing is culture-invariant, fixing bugs under locales like de-DE/fr-FR where `"1.5"` could silently read as `15`. Byte arrays now encode and decode Base64 directly between UTF-8 buffers.
...
- Bug fix: a `null` list element would cause ~2 billion iterations adding `null`s to a list, due to `BeginSubObject` not advancing the reader.
- Bug fix: when writing a string with an unpaired surrogate, `SyncJson.Writer` reserved buffer space for the character's raw UTF-8 form although it actually writes a 6-byte `\uXXXX` escape, causing `IndexOutOfRangeException` if the output buffer had no slack (found by the new fuzz tests).
